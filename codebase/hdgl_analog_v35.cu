...
/* ── Circular reward — reward_accum is a PHASE ANGLE ∈ [0, 2π] on the unit circle. ─────
 * Replaces the old scalar accumulator (acc = DECAY*acc + reward) which could go
 * negative and freeze the gate.  On the circle the gate fires whenever acc enters
 * the "good arc" (cos(acc) > GATE_COS_THRESH), independently of any threshold.
 *
 * Each step:  acc = fmod(acc + reward * dt * REWARD_ANG_RATE, 2π)        (S¹ advance)
 * Gate fires: cos(acc) > GATE_COS_THRESH   ↔   acc ∈ [0, π/3] ∪ [5π/3, 2π]
 * After gate: acc += ARC_KICK (rotate out of good arc → natural refractory period)
 * Pluck:      assigns random acc ∈ [0, π/3] (gate-open position)          (S¹ reset)
 *
 * Reward also injects into phvel so high-reward slots advance phase faster,
 * maintaining amplitude diversity → trit diversity → continuous gate firing.
 *
 * SIGMA_TRIT_GAIN_F: σ-trit→phvel (from conscious_fused_engine.cu DT*0.3*sigma)
 * EXPL_BONUS_F: exploration bonus drives reward when acc is in the "cold" half
 *   bonus = EXPL_BONUS_F * max(0, -cos(acc))  [max at acc=π, zero in good arc]
 * STALL_DETECT_CYCLES: host fires Pluck if no new candidates for N cycles. ─────────── */
#define SIGMA_TRIT_GAIN_F 0.30f    /* σ-trit→phvel (from conscious_fused_engine.cu)   */
#define EXPL_BONUS_F      2.0f     /* exploration bonus in cold half of circle        */
#define REWARD_ANG_RATE   30.0f    /* radians advanced per unit reward per dt         */
#define REWARD_PHASE_GAIN 0.3f     /* reward→phvel: high-reward slots advance faster  */
#define GATE_COS_THRESH   0.5f     /* cos(π/3): gate fires in ±60° arc around 0       */
#define ARC_KICK          1.2f     /* post-gate rotation out of good arc (> π/3)      */
#define PLUCK_ACC_SEED    0.0f     /* Pluck resets acc=0 (cos=1, gate open)           */
#define STALL_DETECT_CYCLES 50     /* host: fire Pluck if no new cands for N cycles   */
...
__global__ void hdgl_field_step_kernel(int step_count)
{
    ...
    /* σ-trit feedback: port of 'phase += DT * 0.3 * sigma' from conscious_fused_engine.cu
     * Computed here using the running sigma_trit from the Markov gate below.
     * sigma_trit is computed AFTER phvel is used, so we use the prior-step trit
     * stored in the ll_state LCG as a proxy.  We compute a per-slot trit now: */
    uint32_t s_trit_lcg = s ^ (uint32_t)(step_count * 1664525u);
    s_trit_lcg = s_trit_lcg * 1664525u + 1013904223u;
    float u_trit = (float)(s_trit_lcg >> 8) * (1.0f / 16777216.0f);
    /* Reuse the same logits as the main Markov gate later (lambda_k = local_amp) */
    float _lk   = local_amp;
    float _lbar_approx = omega + K_COUPLING_F * 0.5f;  /* cheap warp-mean proxy */
    float _ln   = __expf(-1.0f * _lk);
    float _lz   = __expf(-1.0f * fabsf(_lk - _lbar_approx));
    float _lp   = __expf(+1.0f * (_lbar_approx - _lk));
    float _iz   = 1.0f / (_ln + _lz + _lp);
    int prior_trit = (u_trit < _ln*_iz) ? -1 : (u_trit < (_ln+_lz)*_iz ? 0 : +1);

    float dphvel = omega
                 + K_COUPLING_F * sum_sin
                 + 0.15f * gra_sum
                 + KAPPA_PHI * logf(r_h + 1.0f)
                 + SIGMA_TRIT_GAIN_F * (float)prior_trit;  /* σ-trit feedback */
    ...
    /* Exploration bonus on the unit circle: maximum in the "cold" half (acc ≈ π),
     * zero in the good arc (cos(acc) > 0).  Drives reward when the slot is far
     * from the gate, accelerating its return to the firing arc.               */
    float expl_bonus = EXPL_BONUS_F * fmaxf(0.0f, -cosf(acc));

    reward = reward + res_bonus + expl_bonus;
    ...
    /* Circular reward accumulator — advance acc on S¹.                    */
    /* High reward → fast angular advance → more frequent gate firing.     */
    acc = fmodf(acc + reward * dt * REWARD_ANG_RATE, TWO_PI_F);
    if (acc < 0.0f) acc += TWO_PI_F;

    /* Reward→phvel: diversity in reward → diversity in phase → diversity  */
    /* in local_amp (next step) → trit diversity → gate keeps firing.      */
    phvel += REWARD_PHASE_GAIN * reward;
    ...
    if (phi_neg > 0.45f) {
        ...
        if      (R       > 0.6f)  verdict = 2;
        else if (phi_pos > 0.25f) verdict = 1;   /* 0.25: fires with equilibrium trit dist */
        else                      verdict = 0;
    }

    /* Gate fires when acc is in the good arc: cos(acc) > GATE_COS_THRESH  */
    /* ↔ acc ∈ [0, π/3] ∪ [5π/3, 2π].  No threshold drift possible.       */
    int meets_gate = (verdict == 1 && cosf(acc) > GATE_COS_THRESH) ? 1 : 0;
    ...
    if (meets_gate && cluster_size >= CLUSTER_QUORUM) {
        ...
        /* Rotate out of good arc → refractory period before next fire.   */
        g_soa.reward_accum[i] = fmodf(acc + ARC_KICK, TWO_PI_F);
    }
}
...
__global__ void hdgl_reward_inject_kernel(void)
{
    ...
    /* Rotate acc on S¹: LL prime → advance toward gate; composite → retreat. */
    if (flag == 1) { acc = fmodf(acc + 0.5f,                TWO_PI_F); r_h = fminf(r_h * 1.05f, R_MAX_F); }
    else           { acc = fmodf(acc + TWO_PI_F - 0.3f,     TWO_PI_F); r_h = fmaxf(1.0f, r_h * 0.98f); }
...
void hdgl_v33_upload_soa(const DevSoA *h) {
    /* Prefer maximum shared memory carveout (96 KB on sm_75) so the field
     * kernel can run up to 7 concurrent blocks/SM instead of 3 at 48 KB.
     * Static smem = 3×sh_gc/gs/gsg[4][256] + sh_u_mid[8] + 3 scalars ≈ 12.1 KB/block. */
    cudaFuncSetAttribute(hdgl_field_step_kernel,
...
// ============================================================================
// Pluck kernel — Wu Wei re-excitation (random phase diversity + S¹ reset)
// ============================================================================
// Called by host when stall detected (no new candidates for STALL_DETECT_CYCLES).
// Root cause of stalls: amplitude homogenization → lambda_k ≈ lambda_bar for all
// slots → e_zero dominates → sigma_trit=0 → phi_pos < 0.35 → verdict never 1.
// Fix: inject random phase offsets to break symmetry.  Random phases → different
// coupling forces → different amplitude growth rates → restored trit diversity.
// Also resets acc to 0 (gate-open position on S¹) for immediate firing.
// seed_acc is used as a hash perturbation so each Pluck call applies distinct
// random phases (pass st.total_cycles from host to vary per call).
// ============================================================================

__global__ void hdgl_pluck_kernel(float seed_acc)
{
    const int i = blockIdx.x * blockDim.x + threadIdx.x;
    if (i >= g_soa.N) return;

    /* Hash slot index + seed to get a unique random phase for this Pluck call */
    uint32_t h = (uint32_t)i * 2654435761u ^ __float_as_uint(seed_acc);
    h = h * 1664525u + 1013904223u;
    float ph_new = (float)(h >> 8) * (1.0f / 16777216.0f) * TWO_PI_F;

    /* Assign random phase — breaks amplitude homogenization next coupling step */
    g_soa.phase[i] = ph_new;
    g_soa.A_re[i]  = cosf(ph_new);
    g_soa.A_im[i]  = sinf(ph_new);

    /* Assign random acc uniformly on [0, 2π] — keeps slots desynchronized.    */
    /* Do NOT reset all to 0: that would re-synchronize them and create bursts. */
    uint32_t h2 = h * 1664525u + 1013904223u;
    g_soa.reward_accum[i] = (float)(h2 >> 8) * (1.0f / 16777216.0f) * TWO_PI_F;
}

extern "C" {

void hdgl_v35_pluck_kick(int N, int block, float seed_acc, cudaStream_t s) {
    hdgl_pluck_kernel<<<(N + block - 1) / block, block, 0, s>>>(seed_acc);
}
