// ============================================================================
// HDGL Analog v36 — Exploration Fix (self-excitation + σ-trit feedback + LCG entropy)
// ============================================================================
//
// CHANGES FROM v35 (three targeted exploration fixes; all v35 content preserved):
//
//   [EXPLORE-FIX-1] Self-excitation amplitude drive (from conscious_fused_engine.cu)
//     Adds +GAMMA_F*cosf(ph) / +GAMMA_F*sinf(ph) to dA_re/dA_im.
//     Ports: `f_re += c_DT*(-0.5*f_re + cosf(phase))` from fused engine.
//     Effect: isolated equilibrium A* = GAMMA_F/(GAMMA_F+LAMBDA_F) ≈ 0.29
//             — below gate threshold, but prevents runaway amplitude collapse
//             when all neighbors are weak.  Coupling kicks back in once any
//             slot recovers, restarting the lattice.
//
//   [EXPLORE-FIX-2] LCG entropy — breaks LCG periodicity (from conscious_fused_engine.cu curand)
//     Adds step_count ^ (i*16777619) to the LCG seed.
//     Effect: sigma_trit is no longer periodic when acc and ll_state converge
//             → warp ballot remains stochastic throughout long runs.
//
//   [EXPLORE-FIX-3] σ-trit phase-velocity feedback (from conscious_fused_engine.cu + ll_analog.c)
//     Ports: `phase += dt*(lambda_k + 0.3*sigma)` — canonical 0.3 coefficient
//     from both `conscious_fused_engine.cu` line 232 and `ll_analog.c` line 554
//     (`deriv.phase_vel = omega + K*sum_sin + 0.3*wave_mode`).
//     phvel write-back deferred to after warp ballot; stored as phvel+0.3*sigma.
//     Effect: different warp sigma → diverging phvel → diverging phases next step
//             → different lambda_bar distributions → sustained candidate flow.
//
// ============================================================================
...
__global__ void hdgl_field_step_kernel(int step_count)
{
    ...
    /* [EXPLORE-FIX-1] Self-excitation drive — ports conscious_fused_engine.cu  */
    /* dA += +cosf/sinf(phase) → isolated A* ≈ 0.29; prevents global collapse   */
    float dA_re   = -GAMMA_F * A_re + GAMMA_F * cosf(ph);
    float dA_im   = -GAMMA_F * A_im + GAMMA_F * sinf(ph);
    float sum_sin = 0.0f;
    float gra_sum = 0.0f;
    ...
    g_soa.A_im[i]         = A_im;
    g_soa.phase[i]        = ph;
    /* phvel stored below with σ-trit modulation [EXPLORE-FIX-3] */
    ...
    /* [EXPLORE-FIX-2] Entropy: step_count + slot FNV breaks LCG periodicity   */
    /* when acc converges and ll_state enters orbit (was: periodic sigma_trit)  */
    uint32_t lcg = s ^ __float_as_uint(acc) ^ (uint32_t)step_count ^ ((uint32_t)i * 16777619u);
    ...
    /* [EXPLORE-FIX-3] σ-trit velocity feedback — conscious_fused_engine.cu Stage 1 */
    /* phase += dt*(lambda_k + 0.3*sigma)  →  phvel diverges across warps with      */
    /* different sigma_trit → phases stay decorrelated → coupling forces stay alive   */
    g_soa.phase_vel[i] = phvel + 0.3f * (float)sigma_trit;
