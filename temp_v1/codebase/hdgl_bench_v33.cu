...
static void test_field(void) {
    ...
    // Run 10 field steps
    for (int i = 0; i < 10; i++) hdgl_v33_field_step(N, 256, stream);
...
static void benchmark_field(void) {
    ...
    for (int ni = 0; ni < 3; ni++) {
        ...
        // Warmup
        for (int i = 0; i < 5; i++) hdgl_v33_field_step(N, BLK, stream);
        CUDA_CHECK(cudaStreamSynchronize(stream));

        // Timed run
        double t0 = now_sec();
        for (int i = 0; i < STEPS; i++) hdgl_v33_field_step(N, BLK, stream);
        CUDA_CHECK(cudaStreamSynchronize(stream));
        double t1 = now_sec();
