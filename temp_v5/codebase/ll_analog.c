...
/* ── MSVC __int128 compatibility ────────────────────────────────────────────
 * MSVC x64 does not support __int128.  Replace with _umul128 + explicit
 * carry arithmetic.  GCC/Clang: no change needed.                         */
#ifdef _MSC_VER
#include <intrin.h>
/* mul64: 64×64 → 128, returns lo; *hi_out = high 64 bits */
static __forceinline uint64_t _ll_mul64(uint64_t a, uint64_t b, uint64_t *hi_out) {
    return _umul128(a, b, hi_out);
}
/* add3_64: a+b+c → lo; *carry_out = 0,1,2 */
static __forceinline uint64_t _ll_add3(uint64_t a, uint64_t b, uint64_t c, uint64_t *carry_out) {
    uint64_t r = a + b; uint64_t c1 = (r < a) ? 1ULL : 0ULL;
    r += c;             c1 += (r < c) ? 1ULL : 0ULL;
    *carry_out = c1; return r;
}
#endif /* _MSC_VER */
...
#ifdef _MSC_VER
        out[k] = _ll_add3(out[k], hw, carry, &carry);
#else
...
#ifdef _MSC_VER
            uint64_t r = out[k] + c; c = (r < c) ? 1ULL : 0ULL; out[k] = r;
#else
            unsigned __int128 a = (unsigned __int128)out[k] + c;
            out[k] = (uint64_t)a; c = (uint64_t)(a >> 64);
#endif
        }
        carry = c;
    }
...
static void ap_sqr_mersenne(uint64_t *s, size_t n, uint64_t p, uint64_t *tmp) {
    ...
    for (size_t i = 0; i < n; i++) {
        ...
        for (size_t j = i + 1; j < n; j++) {
            uint64_t hi, lo = _ll_mul64(xi, s[j], &hi);
            uint64_t t_lo = lo + tmp[i + j]; uint64_t c1 = (t_lo < lo) ? 1ULL : 0ULL;
            t_lo += carry;                   c1 += (t_lo < carry) ? 1ULL : 0ULL;
            tmp[i + j] = t_lo; carry = hi + c1;
        }
        size_t k = i + n;
        while (carry) {
            uint64_t t = tmp[k] + carry; carry = (t < carry) ? 1ULL : 0ULL;
            tmp[k] = t; k++;
        }
#else
    ...
    for (size_t i = 0; i < n; i++) {
#ifdef _MSC_VER
        uint64_t dhi, dlo = _ll_mul64(s[i], s[i], &dhi);
        uint64_t carry_p3 = tmp[2*i] + dlo;
        uint64_t cp3c     = (carry_p3 < dlo) ? 1ULL : 0ULL;
        tmp[2*i]          = carry_p3;
        carry_p3          = cp3c + dhi; /* ≤ 2^64-1: proved in comments */
        for (size_t k = 2*i + 1; carry_p3; k++) {
            uint64_t t = tmp[k] + carry_p3; carry_p3 = (t < carry_p3) ? 1ULL : 0ULL;
            tmp[k] = t;
        }
#else
