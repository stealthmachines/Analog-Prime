...
//     • Candidate count <= warp-election max (10 * N/32 = 20480)
//
//   TEST 10 — 8D Kuramoto analog LL (CUDA-free):
//     • ll_analog(127,0) → PRIME, ll_analog(131,0) → COMPOSITE, M_89 → PRIME
//     • Verbose readout shows oscillator phase, CV, lock, S(U)
//     • Provides CUDA-free fallback + double-confirmation for GPU path
//
//   TEST 11 — Gate convergence (CPU-only):
//     • Fresh critic, prime-biased and composite-biased state vectors
//     • 100 training cycles: TD(0) observe(prime,+1) + observe(comp,-1)
//     • Assert V(prime) rises, V(comp) < V(prime) — gate biases correctly
//     • Empirically closes LL-residue → reward → gate loop
//
...
static void test_field(void) {
    ...
    for (int i = 0; i < N; i++) tmp[i] = 0.5f;  /* realistic r_h: midrange harmonic residue */
    ...
    /* Warp-election emits at most 1 candidate per warp per step.
     * Theoretical max = steps * (N / WARP_SIZE) = 10 * (65536/32) = 20480. */
    check("candidate count <= warp-max (10 * N/32)", h_cnt <= 10 * (N / 32));
    printf("  candidates promoted = %d  (buffer stores min(%d,%d))\n",
           h_cnt, h_cnt, MAX_CAND_BUF);
...
// ============================================================================
// TEST 9 — hdgl_gpucarry_ll_large: correctness for p~10K (multi-limb range)
// Verifies M_9941 (known Mersenne prime) and M_9949 (composite, not a
// Mersenne prime exponent; next Mersenne prime after M_9941 is M_11213).
// p=9941 → n=ceil(9941/64)=156 limbs, exercises the full multi-limb path.
// ============================================================================

static void test_ll_large(void) {
    printf("\n=== TEST 9: LL large (p~10K, multi-limb) ===\n");

    double t0, t1;

    t0 = now_sec();
    int r1 = hdgl_gpucarry_ll_large(9941);
    t1 = now_sec();
    printf("  M_9941: %s  %.2f s  (expected PRIME)\n",
           r1 ? "PRIME" : "COMPOSITE", t1 - t0);

    t0 = now_sec();
    int r2 = hdgl_gpucarry_ll_large(9949);
    t1 = now_sec();
    printf("  M_9949: %s  %.2f s  (expected COMPOSITE)\n",
           r2 ? "PRIME" : "COMPOSITE", t1 - t0);

    check("M_9941 returns PRIME",     r1 == 1);
    check("M_9949 returns COMPOSITE", r2 == 0);
}

// TEST 10 — 8D Kuramoto analog LL path (CUDA-free fallback + double-confirmation)
// ll_analog() runs exact APA squaring mod 2^p-1 alongside an 8D Kuramoto
// oscillator. Prime: oscillator locks + residue=0.  Composite: residue!=0.
// Tests M_127 (known prime, small), M_131 (not a Mersenne prime exponent),
// and cross-checks M_9941 against the GPU gpucarry result.
// ============================================================================

static void test_analog_ll(void) {
    printf("\n=== TEST 10: 8D Kuramoto analog LL (CUDA-free) ===\n");

    double t0, t1;

    // M_127: smallest 2-digit exponent Mersenne prime (verified)
    t0 = now_sec();
    int r127 = ll_analog(127, 0);
    t1 = now_sec();
    printf("  M_127  : %s  %.3f s  (expected PRIME)\n",
           r127 ? "PRIME" : "COMPOSITE", t1 - t0);

    // M_131: 131 is prime but M_131 is composite (not a Mersenne prime)
    t0 = now_sec();
    int r131 = ll_analog(131, 0);
    t1 = now_sec();
    printf("  M_131  : %s  %.3f s  (expected COMPOSITE)\n",
           r131 ? "PRIME" : "COMPOSITE", t1 - t0);

    // M_89: known Mersenne prime
    t0 = now_sec();
    int r89 = ll_analog(89, 0);
    t1 = now_sec();
    printf("  M_89   : %s  %.3f s  (expected PRIME)\n",
           r89 ? "PRIME" : "COMPOSITE", t1 - t0);

    check("M_127  analog: PRIME",     r127 == 1);
    check("M_131  analog: COMPOSITE", r131 == 0);
    check("M_89   analog: PRIME",     r89  == 1);

    // Verbose readout for M_127 (shows oscillator phase + lock status + S(U))
    printf("  [analog verbose readout for M_127]\n");
    ll_analog(127, 1);
}
...
// ============================================================================
// TEST 11 -- Gate Convergence (CPU-only, no GPU kernel needed)
//
// Verifies that the TD(0) reward loop actually biases the critic gate toward
// prime candidates and away from composites over successive training steps.
//
// Protocol:
//   1. Re-initialise a fresh critic.
//   2. Build a "prime-biased" state: low residue, high coherence, high amp.
//      Build a "composite-biased" state: high residue, low coherence, low amp.
//   3. Record V_prime_before = critic_forward(prime_state).
//   4. Train 100 cycles: observe(prime_state, +1.0) then
//                        observe(composite_state, -1.0).
//   5. Record V_prime_after, V_composite_after.
//   6. Assert V_prime_after  > V_prime_before   (gate opened for primes).
//   7. Assert V_composite_after < V_prime_after  (gate stays closed for composites).
//
// This is the empirical closure of the LL residue → reward → gate loop.
// ============================================================================

static void test_gate_converges(void)
{
    printf("\n=== TEST 11: Gate Convergence (TD0 bias check) ===\n");

    // Fresh critic so previous tests don't influence this one
    critic_init();

    // Prime-like state: residue~0, coherence~1, amp~1, r_h~0.9, acc~0.9
    // Feature indices: 0=residue  1=coherence  2=amp  3=r_h_norm  4=acc_norm
    const float prime_state[CRITIC_IN] = { 0.02f, 0.95f, 0.92f, 0.90f, 0.88f };

    // Composite-like state: residue~1, coherence~0.1, amp~0.2, r_h~0.1, acc~0.05
    const float comp_state[CRITIC_IN]  = { 0.98f, 0.08f, 0.18f, 0.10f, 0.05f };

    float v_prime_before = critic_forward(prime_state);
    float v_comp_before  = critic_forward(comp_state);
    printf("  V(prime) before training = %.4f\n", (double)v_prime_before);
    printf("  V(comp)  before training = %.4f\n", (double)v_comp_before);

    // 5000 training cycles: reward +1 for prime, -1 for composite
    // (scaled from 500 to match CRITIC_LR=0.001; CPU loop, no GPU, still fast)
    for (int i = 0; i < 5000; i++) {
        float td_prime = critic_td_target(+1.0f, prime_state);
        float td_comp  = critic_td_target(-1.0f, comp_state);
        critic_observe(prime_state, td_prime);
        critic_observe(comp_state,  td_comp);
    }

    float v_prime_after = critic_forward(prime_state);
    float v_comp_after  = critic_forward(comp_state);
    printf("  V(prime) after  training = %.4f\n", (double)v_prime_after);
    printf("  V(comp)  after  training = %.4f\n", (double)v_comp_after);
    printf("  delta_prime = %+.4f   delta_comp = %+.4f\n",
           (double)(v_prime_after - v_prime_before),
           (double)(v_comp_after  - v_comp_before));

    check("V(prime) moved up after prime-reward training",
          v_prime_after > v_prime_before);
    check("V(comp) moved down after composite-penalty training",
          v_comp_after < v_comp_before);
    check("V(prime) > V(comp) after training (gate separates prime from composite)",
          v_prime_after > v_comp_after);

    printf("  Gate convergence: TD(0) loop closes LL-residue -> reward -> gate\n");
    critic_print_stats();
}
